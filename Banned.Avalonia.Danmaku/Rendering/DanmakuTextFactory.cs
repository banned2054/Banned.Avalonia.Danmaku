...
internal static class DanmakuTextFactory
{
    ...
    {
        var fontSize    = Math.Max(minimumFontSize, item.FontSize * fontScale);
        var effectScale = Math.Max(0.25, fontSize / Math.Max(1, item.FontSize));
        var foreground = new SolidColorBrush(ToColor(item.Color));
        var fontFamily = new FontFamily(fontFamilyName);
        var fontWeight = isBold ? FontWeight.Bold : FontWeight.Normal;
        var padding    = Math.Max(1, fontSize * 0.12);
        ...
        foreach (var shadow in ShadowOffsetsFor(textEffect, effectScale))
        {
    ...
    private static IEnumerable<ShadowOffset> ShadowOffsetsFor(DanmakuTextEffect effect, double scale)
    {
        return effect switch
        {
            DanmakuTextEffect.Outline =>
            [
                new ShadowOffset(-1.5    * scale, -1.5 * scale, 230),
                new ShadowOffset(0, -1.5 * scale, 230),
                new ShadowOffset(1.5     * scale, -1.5 * scale, 230),
                new ShadowOffset(-1.5    * scale, 0, 230),
                new ShadowOffset(1.5     * scale, 0, 230),
                new ShadowOffset(-1.5    * scale, 1.5 * scale, 230),
                new ShadowOffset(0, 1.5  * scale, 230),
                new ShadowOffset(1.5     * scale, 1.5 * scale, 230)
            ],
            DanmakuTextEffect.Shadow45 => [new ShadowOffset(2.4 * scale, 2.4 * scale, 210)],
            _ =>
            [
                new ShadowOffset(-1    * scale, 0, 245),
                new ShadowOffset(1     * scale, 0, 245),
                new ShadowOffset(0, -1 * scale, 245),
                new ShadowOffset(0, 1  * scale, 245)
            ]
        };
    }
